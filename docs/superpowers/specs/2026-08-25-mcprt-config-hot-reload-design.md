...
| プロセスがstdioのみで起動（HTTPリスナーなし） | `watchSIGHUP`自体をspawnしない。`SIGHUP`はOSのデフォルト挙動（プロセス終了）のまま — これは本設計導入前からの既存動作であり退行ではない。実装確認・レビュー済み（2026-08-29実装プランのTask 5レビューで裏付け） |
| stdio+http混在構成で`SIGHUP`受信 | HTTPの新規接続だけ新世代に切り替わる。既存のstdio 1セッションは旧世代のまま動き続けるが、旧世代のbackend接続は`drainタイムアウト`（デフォルト5分）経過後に強制クローズされるため、その時点以降stdioセッションの呼び出しは全て失敗する（プロセス再起動が必要）。ログにその旨を明記する |
| drainタイムアウト経過時、旧backend接続がまだ使用中のセッションがある | 強制クローズ。以降その呼び出しは既存の「切断済みbackend」エラーと同じ扱いになる |
| 短時間に`SIGHUP`が連続して届く | 実装では単一goroutine＋buffer-1チャネルで直列化しており、各reloadは前のreloadの`Swap`/`scheduleDrain`処理が終わってから次を開始する（本文の「独立して並行構築してよい」という記述より安全側に倒した意図的な実装判断。2026-08-29実装プランのTask 5レビューで妥当と確認済み） |
